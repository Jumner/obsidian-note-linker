...
impl<'m> LinkFinderMatchingResult<'m> {
	fn find_matches(note: &'m mut Note, target_note: &'m Note) -> Self {
		...
		let regex_matches: Vec<RegexMatch> = build_link_finder(target_note)
			...
			.filter_map(|capture_result| match capture_result {
...
impl<'m> Into<Vec<LinkMatch>> for LinkFinderMatchingResult<'m> {
	fn into(self) -> Vec<LinkMatch> {
		...
		let text_link_matches: Vec<LinkMatch> = self
			.regex_matches
			.into_iter()
			.map(|regex_match: RegexMatch| LinkMatch::new_from_match(&regex_match, note, target_note))
			.collect();
		text_link_matches
	}
}
...
fn concat_as_regex_string(strings: &[String]) -> String {
	strings
		...
		.fold("".to_string(), |prev, (index, current)| {
			return if index == 0 {
				format!("((?i)\\b{}\\b)", current)
			} else {
				format!("{}|((?i)\\b{}\\b)", prev, current)
			};
		})
		.add("")
}
...
fn build_link_finder(target_note: &Note) -> LinkFinder {
	let mut escaped_search_strings: Vec<String> = target_note
		.aliases_vec()
		.iter()
		.map(|alias| escape(alias).to_string())
		.collect();
...
fn find_link_matches(target_note: &Note, note_to_check: &mut Note) -> Option<Vec<LinkMatch>> {
	if !&target_note.title().eq(&note_to_check.title()) {
		let link_finder_match = LinkFinderMatchingResult::find_matches(note_to_check, target_note);
...
fn merge_link_match_into_link_matches(
	mut merged_link_matches: Vec<LinkMatch>,
...
pub fn find_links(
	note_to_check: &mut Note,
	target_note_candidates: &[Note],
) -> Option<LinkFinderResult> {
	let link_matches: Vec<LinkMatch> = target_note_candidates
		.iter()
	...
	if !link_matches.is_empty() {
		return Some(LinkFinderResult::new(note_to_check.clone(), link_matches));
	}
